...
namespace XPlaneGenConsole
{
    ...
    {
        private static MethodInfo ConversionMethod = typeof(Extensions).GetMethods (BindingFlags.Public | BindingFlags.Static)
			.Where (m => m.Name.Equals ("As") && m.GetGenericArguments ().Length == 0).First ();
            ...
            where T : BinaryDatapoint, new()
        {
            ...
            {
				methodCall = null;
                var index = Expression.ArrayAccess(nums, Expression.Constant(info.Field.First().Index));
                ...
                if (info.Format != null)
                {
                    ...
                    {
                        // Is supported unit
                        methodCall = CallFrom(CallConvert(index), info.Type, info.Format.value);
                    ...
                    else if (info.Format.IsCustomized)
                    {
                        ...
                        methodCall = CallFrom(invoke, info.Type);
                    }
                    else if(info.Format.Style != NumberStyles.None)
                    {
                        methodCall = CallConvert(index, info.Type);
                    }
                    else
                    {
                ...
                else
                {
					...
					else if (info.Field.Count() == 2)
                    {
                        ...
                        var left = CallConvert(index, info.Type);
